...
public static class ABC_Utilities {
    ...
    public static float EntityDistanceFromGround(Transform Entity) {
        ...
        List<RaycastHit> hit = Physics.RaycastAll(origin, direction, 10f).ToList();

        if (hit.Where(h => ABC_Utilities.IsStaticABCEntity(h.transform.gameObject) == false).OrderBy(x => x.distance).Count() > 0) {
    ...
    public static bool EntityIsGrounded(ABC_IEntity Entity) {
        ...
        Vector3 pos = Entity.transform.position + Vector3.up * (radius * 0.6f);
